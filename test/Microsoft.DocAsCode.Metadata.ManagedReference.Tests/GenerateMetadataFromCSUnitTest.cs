...
public class TestAttribute : Attribute", @class.Syntax.Content[SyntaxLanguage.CSharp]);
            ...
            Assert.NotNull(ctor);
            Assert.Equal(@"public TestAttribute(object obj)", ctor.Syntax.Content[SyntaxLanguage.CSharp]);

            var property = @class.Items[1];
            Assert.NotNull(property);
            Assert.Equal(@"public object Property { get; set; }", property.Syntax.Content[SyntaxLanguage.CSharp]);
        }
